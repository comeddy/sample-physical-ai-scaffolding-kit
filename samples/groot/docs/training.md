...
S3にアップロードしたファイルをデータとして利用する場合は、以下の様に書き込みが発生するため事前にパーミッションを変更してから、トレーニングのコマンドを実行してください。
サンプルデータがLustre上の `/fsx/ubuntu/` 以下にある場合は、権限の変更は不要です。
...
トレーニングのジョブは、環境変数でパラメータをカスタマイズできます。
